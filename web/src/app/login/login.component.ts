...
export class LoginComponent {
 ...
 ngOnInit() {  
    ...
    const codAKD = this.route.snapshot.paramMap.get('codAKD');

                // console.log('codAKD',codAKD);
                    if(codAKD) {   
                      localStorage.setItem('token', codAKD);
                      this.loginService.validateTokenAfterLogin(codAKD).subscribe({
                      next: (validationResponse) => {
                        if(validationResponse.message=="Token válido") {
